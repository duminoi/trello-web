...
export default function BoardContent({ board }) {
  ...
  const handleDragOver = (event) => {
    ...
    if (activeColumn._id !== overColumn._id) {
      setOrderedColumns((prevColumns) => {
        ...
        // Logic tính toán "cardIndex mới" (trên hoặc dưới của overCard) (của thư viện DndKit)
        let newCardIndex
  ...
  const handleDragEnd = (event) => {
    ...
    if (activeDragItemType === ACTIVE_DRAG_ITEM_TYPE.CARD) {
      ...
      if (activeColumn._id !== overColumn._id) {
        console.log('hành động kéo khi khác column')
      } else {
        console.log('hành động kéo khi cùng column')
      }
    }
  ...
  return (
    <DndContext
      // Cảm biến (xem kĩ ở video 30)
      sensors={mySensors}
      // Thuật toán phát hiện va chạm (nếu không có nó thì card với cover lớn sẽ không kéo qua column được vì lúc này nó đang bị conflict giữa column và card), chúng ta sẽ dùng closestCorners thay vì closestCenter
